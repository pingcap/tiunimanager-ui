...
function readWord(input, result) {
  for (;;) {
    if (input.next != 95 /* Underscore */ && !isAlpha(input.next)) break
...
function readWordOrQuoted(input) {
  if (
    ...
    input.next == 34 /* DoubleQuote */ ||
    input.next == 96 /* Backtick */
  ) {
...
function readBits(input, endQuote) {
  while (input.next == 48 /* _0 */ || input.next == 49 /* _1 */) input.advance()
...
function eol(input) {
  while (!((input.next < 0 || input.next == 10) /* Newline */)) input.advance()
}
...
function tokensFor(d) {
  return new ExternalTokenizer((input) => {
    ...
    if (inString(next, Space)) {
      ...
      next == 36 /* Dollar */ &&
      input.next == 36 /* Dollar */ &&
      d.doubleDollarStrings
    ) {
      readDoubleDollarLiteral(input)
      input.acceptToken(String$1)
    } else if (
      next == 39 /* SingleQuote */ ||
      (next == 34 /* DoubleQuote */ && d.doubleQuotedStrings)
    ) {
      ...
      (next == 101 /* e */ || next == 69) /* E */ &&
      input.next == 39 /* SingleQuote */
    ) {
      input.advance()
      readLiteral(input, 39 /* SingleQuote */, true)
    } else if (
      (next == 110 /* n */ || next == 78) /* N */ &&
      input.next == 39 /* SingleQuote */ &&
      d.charSetCasts
    ) {
      input.advance()
      ...
      (next == 48 /* _0 */ &&
        (input.next == 120 /* x */ || input.next == 88)) /* X */ ||
      ((next == 120 /* x */ || next == 88) /* X */ &&
        input.next == 39) /* SingleQuote */
    ) {
      let quoted = input.next == 39 /* SingleQuote */
      input.advance()
      while (isHexDigit(input.next)) input.advance()
      if (quoted && input.next == 39 /* SingleQuote */) input.advance()
